...
void checkFindRefs(llvm::StringRef Test, bool UseIndex = false) {
  ...
  for (const auto &[R, Context] : T.rangesWithPayload("def"))
    ExpectedLocations.push_back(AllOf(rangeIs(R), contextIs(Context),
...
TEST(FindReferences, WithinAST) {
  const char *Tests[] = {
      ...
      // FIXME Macro occurrences don't get a context yet
      R"cpp(// Macros
...
TEST(FindReferences, ConceptReq) {
  constexpr llvm::StringLiteral Code = R"cpp(
    ...
    template <class T>
    concept IsSmallPtr = requires(T x) {
      // FIXME Should probably report IsSmallPtr as context here
      { *x } -> [[IsSmal^l]];
    };
  )cpp";
  checkFindRefs(Code);
...
TEST(FindReferences, IncludeOverrides) {
  llvm::StringRef Test =
      R"cpp(
        ...
        public:
          void $overridedecl(Derived)[[func]]() override;
        };
        void Derived::$overridedef[[func]]() {}
        class Derived2 : public Base {
          void $overridedef(Derived2)[[func]]() override {}
        };
  ...
  EXPECT_THAT(
      ...
      ElementsAre(rangeIs(Main.range()),
                  AllOf(rangeIs(IndexedMain.range("decl")),
                        attrsAre(ReferencesResult::Declaration |
                                 ReferencesResult::Definition)),
                  AllOf(rangeIs(IndexedMain.range("bar")), contextIs("bar")),
                  AllOf(rangeIs(IndexedMain.range("S")), contextIs("S::bar")),
                  AllOf(rangeIs(IndexedMain.range("N")), contextIs("N::bar"))
                  ));
  auto LimitRefs =
